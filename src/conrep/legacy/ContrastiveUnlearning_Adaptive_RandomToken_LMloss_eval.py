...
from llm22vec.openunlearn_wrapper import LLM2Vec2CausalLM
import torch
from torch import nn
openunlearn_src_path = os.path.abspath("open_unlearning/src")
print("✅ OpenUnlearn src path added:", openunlearn_src_path)
sys.path.append(openunlearn_src_path)

from evals.tofu import TOFUEvaluator as openunlearn_evaluate
...
class CustomArguments:
    ...
    eval_cfg_path: Optional[str] = field(
        default=None,
        metadata={"help": "Path to OpenUnlearn evaluation config YAML (e.g., tofu/default.yaml)"}
    )
...
class ContrastiveUnlearnTrainer(Trainer):
    ...
    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval"):
        # Step 1: 将 LLM2Vec 封装为 OpenUnlearn 风格的 causal LM
        wrapped_model = LLM2Vec2CausalLM(self.model)

        # Step 2: 加载 open-unlearning 的 eval config
        if GlobalHydra.instance().is_initialized():
            GlobalHydra.instance().clear()

        eval_cfg_path = self.eval_cfg_path
        config_dir = os.path.abspath(os.path.join(eval_cfg_path, "../../"))  # ➜ open_unlearning/configs
        config_dir = os.path.relpath(config_dir)
        config_name="experiment/eval_config"
        print("[DEBUG] eval_cfg_path:", eval_cfg_path)
        print("[DEBUG] config_dir:", config_dir)
        print("[DEBUG] config_name:", config_name)
        print("[DEBUG] files in config_dir:", os.listdir(config_dir))

        with initialize(config_path=config_dir, version_base=None):
            cfg = compose(config_name=config_name)
        # Step 3: 设置动态参数，如模型路径和 retain log
        cfg.pretrained_model_name_or_path = self.args.output_dir
        #cfg.retain_logs_path = "/path/to/retain_log.json"  # 必须事先准备好 retain logs
        #cfg.task_name = "SAMPLE_EVAL"
        print("[DEBUG] Loaded eval_cfg.metrics keys:", list(cfg.get("metrics", {}).keys()))
        cfg.tokenizer_name_or_path = wrapped_model.tokenizer.name_or_path
        
        # Step 4: 调用 evaluation
        result = openunlearn_evaluate(cfg, model=wrapped_model)

        # Step 5: 保存最优 checkpoint
        metric = result["metrics"].get("forget_Q_A_ROUGE", None)
        if metric:
            self.state.best_metric = max(metric, getattr(self.state, "best_metric", float("-inf")))
            if self.state.best_metric == metric:
...
def main():
    ...
    trainer = ContrastiveUnlearnTrainer(
        ...
        eval_cfg_path=custom_args.eval_cfg_path,
        tokenizer= tokenizer
    )
