...
LM_WEIGHTS = [0.01, 0.1, 0]
FORGET_WEIGHTS = [0.1,0.2,0.3,0.4,0.45,0.5,0.7]
LEARNING_RATES = [7e-6,8e-6,1e-5,2e-5]
GAMMAS = [0.1,0.2,0.5,1,2]
EPOCHS = [3,4,5,6]
...
FORGET_CSV = "/gs/bs/tga-TDSAI/h1kkk/unlearn/embedders/LLM2Vec/llm2vec/UnlearnData/easy_QA_PMC_forget100_state.csv"

# Template for evaluation configs
RETAIN_EVAL_TEMPLATE = json.load(open("/gs/bs/tga-TDSAI/h1kkk/unlearn/embedders/LLM2Vec/llm2vec/unlearn_eval/batchedEval_PMC_retain.json"))
FORGET_EVAL_TEMPLATE = json.load(open("/gs/bs/tga-TDSAI/h1kkk/unlearn/embedders/LLM2Vec/llm2vec/unlearn_eval/batchedEval_PMC_forget.json"))

CONFIG_DIR = Path("/gs/bs/tga-TDSAI/h1kkk/unlearn/embedders/LLM2Vec/llm2vec/grid_search/configs")
JOB_DIR = Path("/gs/bs/tga-TDSAI/h1kkk/unlearn/embedders/LLM2Vec/llm2vec/grid_search/jobs")
...
def main():
    for name, ckpt in MODEL_CHECKPOINTS.items():
        for lm_w, fw, lr, gm, epochs in itertools.product(LM_WEIGHTS, FORGET_WEIGHTS, LEARNING_RATES, GAMMAS, EPOCHS):
            exp_name = f"{name}_lm{lm_w}_fw{fw}_lr{lr}_gm{gm}_epochs{epochs}"
            ...
            train_cfg.update({
                ...
                "output_dir": f"/gs/bs/tga-TDSAI/h1kkk/unlearn/embedders/LLM2Vec/llm2vec/grid_search/results/{exp_name}/model",
            })
            ...
            eval_retain.update({"model_path": model_out, "output_dir": f"/gs/bs/tga-TDSAI/h1kkk/unlearn/embedders/LLM2Vec/llm2vec/grid_search/results/{exp_name}/eval_retain"})
            eval_forget.update({"model_path": model_out, "output_dir": f"/gs/bs/tga-TDSAI/h1kkk/unlearn/embedders/LLM2Vec/llm2vec/grid_search/results/{exp_name}/eval_forget"})
            ...
            with open(job_script, "w") as f:
                ...
                f.write("python /gs/bs/tga-TDSAI/h1kkk/unlearn/embedders/LLM2Vec/llm2vec/unlearn_eval/batchedEval_loglikelihood.py {}\n".format(retain_cfg_path))
