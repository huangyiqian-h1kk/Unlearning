...
FORGET_CSV = "/gs/bs/tga-TDSAI/h1kkk/unlearn/embedders/LLM2Vec/llm2vec/UnlearnData/easy_QACelebrityDeath.csv"
...
CONFIG_DIR = Path("grid_search_death/configs")
JOB_DIR = Path("grid_search_death/jobs")
    ...
    for name, ckpt in MODEL_CHECKPOINTS.items():
        for lm_w, fw, lr, gm, epochs in itertools.product(LM_WEIGHTS, FORGET_WEIGHTS, LEARNING_RATES, GAMMAS, EPOCHS):
            ...
            train_cfg.update({
                ...
                "output_dir": f"grid_search_death/results/{exp_name}/model",
            })
            ...
            eval_forget.update({"model_path": model_out, "output_dir": f"grid_search_death/results/{exp_name}/eval_forget"})
