...
LM_WEIGHTS = [0, 0.01]
FORGET_WEIGHTS = [0.4,0.5]
LEARNING_RATES = [7e-6,1e-5]
GAMMAS = [1]

# Paths to training CSVs (edit accordingly)
RETAIN_CSV = "/gs/bs/tga-TDSAI/h1kkk/unlearn/embedders/LLM2Vec/llm2vec/UnlearnData/retain900_note_archive_patient.csv"
...
CONFIG_DIR = Path("grid_search_retainAltered/configs")
JOB_DIR = Path("grid_search_retainAltered/jobs")
    ...
    for name, ckpt in MODEL_CHECKPOINTS.items():
        for lm_w, fw, lr, gm in itertools.product(LM_WEIGHTS, FORGET_WEIGHTS, LEARNING_RATES, GAMMAS):
            ...
            train_cfg.update({
                ...
                "lm_weight": lm_w,
                "gamma": gm,
                "output_dir": f"retainAltered/results/{exp_name}/model",
            })
            ...
            eval_retain.update({"model_path": model_out, "output_dir": f"retainAltered/results/{exp_name}/eval_retain"})
            eval_forget.update({"model_path": model_out, "output_dir": f"retainAltered/results/{exp_name}/eval_forget"})
            ...
            with open(job_script, "w") as f:
                f.write(f"#!/bin/sh\n")
                f.write("#$-cwd\n#$-l gpu_1=1\n#$-l h_rt=24:00:00\n#$-p -3\n")
                f.write(f"#$-N retainAltered_gs_{exp_name}\n\n")
