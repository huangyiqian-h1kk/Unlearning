...
class DefaultCollator:
    ...
    def __init__(self, model: LLM2Vec, num_ctrl_views: int = 1) -> None:
        self.model = model
        self.tokenizer = model.tokenizer
        self.num_ctrl_views = num_ctrl_views
    ...
    def __call__(self, batch) -> Dict[str, torch.Tensor]:
        ...
        forget_features = {k: align_features[k][:-1] for k in align_features.keys()}
        random_features = {k: align_features[k][-1] for k in align_features.keys()}

        # 生成 num_ctrl_views 个 N(id, attr) 视图（每次 swap 都有不同的随机掩码）
        ctrl_features_list = []
        for _ in range(self.num_ctrl_views):
            ctrl_v = swap_tokens_batch_vectorized(forget_features, random_features, self.tokenizer, swap_ratio=0.7)
            ctrl_features_list.append(ctrl_v)

        # 返回第三个位置为“列表”，后续 compute_loss 会逐个前向拿 embedding
        return retain_features, forget_features, ctrl_features_list
...
def l_con_forget_multi(retain_embs, forget_embs, control_vec_list, temp=0.1):
    """
    retain_embs: [R, d]
    forget_embs: [F, d]     —— 每行一个 forget pivot 的表示
    control_vec_list: list of [F, d] —— 与 forget 一一对应的 N 视图，长度 = V（= n_augment）
    设计：对每个 forget 行 i，仅把它“自己的 V 个 N 视图”作为正样；
          其它 controls（属于别的 i）、全部其它 forget（跨 pivot）、全部 retain 作为负样。
    """
    device = forget_embs.device
    F = forget_embs.size(0)
    R = retain_embs.size(0)
    V = len(control_vec_list)

    f = torch.nn.functional.normalize(forget_embs, p=2, dim=1)
    r = torch.nn.functional.normalize(retain_embs, p=2, dim=1)
    ctrls = torch.cat([torch.nn.functional.normalize(c, p=2, dim=1) for c in control_vec_list], dim=0)  # [V*F, d]

    # targets = [controls(V*F), forget(F), retain(R)]
    targets = torch.cat([ctrls, f, r], dim=0)                 # [(V*F)+F+R, d]
    sim = (f @ targets.T) / temp                              # [F, (V*F)+F+R]

    pos = torch.zeros_like(sim, dtype=torch.bool, device=device)
    neg = torch.ones_like(sim, dtype=torch.bool, device=device)  # 先全负，正样位置再清掉

    ar = torch.arange(F, device=device)
    # 正样：每个 i 的 V 个 control 列索引为 k*F + i
    for k in range(V):
        cols = k * F + ar
        pos[ar, cols] = True
        neg[ar, cols] = False

    # 其它 controls（不属于本 i）、其它 forget（含自身列是否排除不敏感，留在负样可更稳）、全部 retain 都保持为负
    exp_sim = torch.exp(sim)
    eps = 1e-12
    pos_sum = (exp_sim * pos).sum(dim=1) + eps
    neg_sum = (exp_sim * neg).sum(dim=1) + eps
    return (-torch.log(pos_sum / (pos_sum + neg_sum))).mean()
...
class ContrastiveUnlearnTrainer(Trainer):
    ...
    def compute_loss(
        ...
        l_con_f = l_con_forget_multi(r_reps, f_reps, control_vec_list, temp=self.temp)
...
def main():
    ...
    data_collator = DefaultCollator(model, num_ctrl_views=custom_args.n_augment)
