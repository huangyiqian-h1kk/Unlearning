...
class LLM2Vec(nn.Module):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        config = AutoConfig.from_pretrained(base_model_name_or_path, token=access_token)
        config_class_name = config.__class__.__name__

        model_class = cls._get_model_class(
            config_class_name, enable_bidirectional=enable_bidirectional
        )
        model = model_class.from_pretrained(base_model_name_or_path, token=access_token, **kwargs)
    ...
    def forward(self, sentence_feature: Dict[str, Tensor]):
        ...
        return self.get_pooling(sentence_feature, last_hidden_state)
