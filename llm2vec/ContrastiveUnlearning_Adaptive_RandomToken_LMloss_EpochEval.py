...
class CustomArguments:
    ...
    retain_eval_config: Optional[str] = field(
        default=None, metadata={"help": "Path to evaluation config for retain set"}
    )

    forget_eval_config: Optional[str] = field(
        default=None, metadata={"help": "Path to evaluation config for forget set"}
    )
...
class EvaluationCallback(TrainerCallback):
    def __init__(self, retain_cfg: Optional[str] = None, forget_cfg: Optional[str] = None):
        self.retain_cfg = retain_cfg
        self.forget_cfg = forget_cfg

    def _run_eval(self, cfg_path: str, model):
        try:
            from unlearn_eval.batchedEval_loglikelihood import evaluate

            with open(cfg_path) as f:
                cfg = json.load(f)

            print(f"\nRunning evaluation with {cfg_path}")
            results = evaluate(cfg, model.tokenizer, model.model)
            print(f"Results: {results}")
        except Exception as e:
            print(f"Evaluation failed for {cfg_path}: {e}")
    ...
    def on_epoch_end(self, args, state, control, **kwargs):
        ...
        if model is None:
            return
        if self.retain_cfg:
            self._run_eval(self.retain_cfg, model)
        if self.forget_cfg:
            self._run_eval(self.forget_cfg, model)
...
def main():
    ...
    if config_path is not None:
        if custom_args.retain_eval_config is None:
            guess = config_path.replace("train_", "retain_")
            if os.path.exists(guess):
                custom_args.retain_eval_config = guess
        if custom_args.forget_eval_config is None:
            guess = config_path.replace("train_", "forget_")
            if os.path.exists(guess):
                custom_args.forget_eval_config = guess
    ...
    if custom_args.retain_eval_config or custom_args.forget_eval_config:
        trainer.add_callback(EvaluationCallback(custom_args.retain_eval_config, custom_args.forget_eval_config))
