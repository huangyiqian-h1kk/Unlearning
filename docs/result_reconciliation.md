...
Each reconciliation row now distinguishes experiment- and cell-scoped status, contains deterministic JSON raw inputs for numeric or masked reconstructed cells, and identifies both method and baseline metric sources. MCQ rows additionally cite `results/paper/mcq_dataset_inventory.json`. Manuscript-transcription status is attached only to the disputed cells: Deaths/Llama-2 ConRep `F-ATT` and `F-IDeq`, Diagnosis/Llama-2 NPO `F-ATT`, and PMC ConRep `ATT-R`.
